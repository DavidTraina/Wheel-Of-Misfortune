...
char **prune_word_list(char **words, int len, int *words_remaining) {
    // read_words puts NULL in the last index. words is dynamically allocated.
    int max_pruned = 10000;
    char **pruned_words = malloc(sizeof(char *) * (max_pruned + 1));
    if (pruned_words == NULL) {
        perror("malloc");
        exit(1);
    }
    int num_pruned = 0;
    ...
    while (words[i] != NULL) {
        ...
        if (strlen(word) == len) {
            if (num_pruned == max_pruned) {
                max_pruned += 10000;
                pruned_words = realloc(pruned_words, sizeof(char *) * (max_pruned + 1));
                if (pruned_words == NULL) {
                    perror("realloc");
                    exit(1);
                }
            }
            pruned_words[num_pruned] = word;
            num_pruned++;
        }
        i++;
    }
    pruned_words = realloc(pruned_words, sizeof(char *) * (num_pruned + 1));
    if (pruned_words == NULL) {
        perror("realloc");
        exit(1);
    }
    pruned_words[num_pruned] = NULL;
    *words_remaining = num_pruned;
    return pruned_words;
}
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    int num_pruned = 0;
    char **pruned_words = NULL;
    while (num_pruned < 1) {
        printf("Length of words to use? ");
        fflush(stdout);

        if (fgets(input_buffer, BUF_SIZE, stdin) == NULL) {
            perror("fgets"); // fgets sets errno
            exit(1);
        }

        *len = strtol(input_buffer, NULL,
                      10); // Will return 0 on error/non-numeric input and no words length 0. Trailing non-numerals is ignored.
        pruned_words = prune_word_list(words, *len, &num_pruned);
        if (num_pruned < 1) {
            printf("There are no words of that length.\n");
        }
    }
